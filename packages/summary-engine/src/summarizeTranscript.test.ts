...
describe("summary engine", () => {
    ...
    const summary = await summarizeTranscript(
      ...
      },
      {
        async generate(prompt) {
          ...
          expect(prompt).toContain("Safety, People, Quality, Delivery, and Cost Updates");
          return {
  ...
  it("includes the selected meeting type and strict schema requirements in the summary prompt", () => {
    ...
    expect(prompt).toContain("For standard recaps, model this structure in executiveRecap:");
  ...
  it("builds standard recap prompts around the WGS MinutesBot executive recap structure", () => {
    const prompt = buildSummaryPrompt({
