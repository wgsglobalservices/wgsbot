...
describe("admin test actions", () => {
  ...
  it("rejects sample recap email recipients outside the allowed domains", async () => {
    const external = await post("/api/admin/send-test-summary-email", env(), { to: "victim@external-corp.com" });

    expect(external.status).toBe(400);
    await expect(external.json()).resolves.toEqual({
      ok: false,
      message: "Test recipient must belong to an allowed domain"
    });
  });
  ...
  it("sends a rendered sample recap email to the specified recipient without exposing secrets", async () => {
    ...
    const response = await post("/api/admin/send-test-summary-email", testEnv, { to: "Reviewer@Company.COM" });
